...
To configure the sidecar spec, we use the `filer-sidecar-injector-json` configmap object, which works as the model / structure that we mutate and inject into the `pod` spec. 

We need:
1. The bucket url: taken from the `filers-list` cm that is read in at injector startup.
2. Bucket name: taken from the `existing-shares` cm and is put through the same hashing function used when creating the bucket. This gives 1:1 mapping for the particular svm.
3. Secret and Access key: live in a secret in the user's namespace.
If any of these key values are empty then the sidecar will skip as the mounting would just fail.

The `filers-list` and `filer-sidecar-injector-json` configmaps should be located in the same namespace as the injector.
...
Create a PR with the `auto-deploy` label, and after the image has successfully pushed to the ACR, go to the `netapp` application in argocd and turn off autosync and update the `filer-sidecar-injector` image tag with the pushed image. It's also possible to use the `latest` tag for rapid testing. Start up a notebook and ensure it patches correctly.  Note, updating the CMs should be done in the ArgoCD Manifests repo as well as the cluster. 
